...
function Login(){
    ...
    if (($('#Name').val().length==0) && ($('#Password').val().length==0)){
        ...
        let Send_Password = Generate_Hash($('#Password').val());
        $.ajax({
            url: URL_str,
            type: 'post',
            data: {"mode":"Site_Login","name" : $('#Name').val() ,"pass" : Send_Password},
            dataType: 'json'
            }).done(function(data){
            /* 通信成功時 */
            ...
            if (data_json[0] == "Not Found Account"){
                ...
                content.style.display = "none";
                        ...
                        // cookieが使えるか確認
                var Save = Save_Account(Send_Password);
...
function Save_Account(Send_Password){
    var Username_temp = String($('#Name').val());
    var Password_temp = String(Send_Password);
...
function Acodion_Menu(){
  ...
  function Generate_Hash(Password){
    ...
    let Generated_hash = hash.toString(CryptoJS.enc.Hex);
